...
1..$num | foreach {
	$date=Get-Date
	new-vm -vmhost $esxi_host_ip -name $vm_prefix_name$_ -datastore VIRTUALSTOR  -Template "win7-test-01" -RunAsync:$true
...
# report
.\Use-Culture.ps1 en-US {get-task} | export-csv "$($vm_prefix_name)_report-clone-vm-general-purpose-profile-async.csv"
Write-Host "done!"